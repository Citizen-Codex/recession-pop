...
<div class="chart-container">
	...
	>
		<Html>
			<!-- Main axes container -->
			<div class="opacity-0 transition-opacity duration-500" style:opacity={inView ? 1 : 0}>
				<!-- X axis -->
				...
				{#if gdpYScale}
					<div style="transition: opacity 500ms;" style:opacity={scrollIndex === 7 ? 1 : 0}>
						<AxisYRight
							scale={gdpYScale}
				...
				{#if unemYScale}
					<div style="transition: opacity 500ms;" style:opacity={scrollIndex === 8 ? 1 : 0}>
						<AxisYRight scale={unemYScale} axisTitle="Unemployment rate (%)" titleDisplace={35} />
					</div>
				{/if}
			</div></Html
		>
